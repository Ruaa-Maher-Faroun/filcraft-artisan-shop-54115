    ...
    <section id="showcase" className="py-20 bg-secondary/20">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-foreground mb-4">{t.showcase.heading}</h2>
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">{t.showcase.subheading}</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {t.showcase.products.map((product, index) => (
            <Card key={index} className="border-border hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden">
              <div className="aspect-[4/3] overflow-hidden">
                <img src={images[index]} alt={product.title} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
              </div>
              <CardHeader>
              ...
              <CardFooter>
                <Button className="w-full bg-primary hover:bg-accent text-primary-foreground">{t.showcase.viewDetails}</Button>
              </CardFooter>
            </Card>
          ))}
        </div>
